...
def plot_label(path, label):
    ...
    # .item() 是一个用于将一个只包含单个元素的张量转换为Python标量（如整数或浮点数）的方法
    plt.imshow(x[label])
    plt.show()
...
def plot_full_img(path, b_scan_idx=0):
    ...
    # .subplots(1,2)创建一个包含1行和2列的图形子图布局的函数，返回一个包含两个子图对象的元组
    img, mask = get_valid_img_seg_reimpl(x)

    # 从一个对象中提取有效图像和分割数据
    axes[0].imshow(mask[:, :, b_scan_idx], cmap=plt.cm.jet, vmax=9)
    # 在第一个子图中显示 mask 切片数据，cmap=plt.cm.jet 指定使用"jet"颜色映射（热图）来可视化图像，vmax=9 设置最大值为9
...
def test_slicing(path):
    ...
    for b in range(x.shape[2]):
        for col in range(col_num):
            assert torch.all(
                x[:, col * 12:(col + 1) * 12, b] == slices[b * col_num + col]), f"slice {col} in image {b} is off"
    sum_scans = sum([e.shape[1] for e in slices])
    assert x.shape[1] // 12 * 12 * x.shape[
        2] == sum_scans, f"The number of a scans in samples {sum_scans} and the number of a" \
...
class DataPreprocessor():
    ...
    def preprocess(self):
        ...
        for img_num in range(len(self.img_files)):
            ...
            # 假设 self.img_files[img_num] 包含文件路径和文件名
            file_name = self.img_files[img_num].split("/")[-1]
            match = re.search(r'_(\d+)\.', file_name)  # 正则表达式匹配数字部分
            if match:
                subject_num = int(match.group(1))
            else:
                # 如果无法找到匹配的数字部分，进行适当的错误处理
                print("Could not find a valid subject number.")
...
class DataPreprocessorUMN():
    ...
    def __init__(self, data_path, dest_path, slicing: Callable[[torch.Tensor], list], is_mat=False):
        ...
        self.slicing = slicing  # 用于切割图像的函数 slicing
        self.is_mat = is_mat  # 加载指定的 .mat 格式数据集文件并提取图像和相应的掩码
        self.img_slice_nums = {}  # 确定每个图像有多少个切片，并将这些信息存储在 img_slice_nums 字典
        # determine samples_per_file
        for i in range(29):
...
if __name__ == "__main__":
    ...
    parser.add_argument("dest_path", type=str)  # 处理后的数据路径
    parser.add_argument("--dataset", type=str)
    parser.add_argument("--is_mat", type=bool, default=True)
    parser.add_argument("--full_bscan", type=bool, default=True)
    parser.add_argument("--extract_dataset", choices=["labelled_dataset", "unlabelled_dataset"],
                        default="labelled_dataset")
    args = parser.parse_args()  # 解析脚本传递的命令行参数，并将值存储在 args 对象中
    ...
    if args.dataset == "UMN":
        ...
        processor = DataPreprocessor(args.data_path, args.dest_path, slice_to_bscans, is_mat=args.is_mat,
                                     labelled_dataset=labelled_dataset)
    else:
        processor = DataPreprocessor(args.data_path, args.dest_path, slicing, is_mat=args.is_mat,
                                     labelled_dataset=labelled_dataset)
